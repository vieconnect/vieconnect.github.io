...
// --- HÀM TỰ ĐỘNG MỞ KHÓA (BACKEND LOGIC TRÊN FRONTEND) ---
function autoUnlockTimer(username) {
    const delayTime = 60000; // 60 giây (Thay đổi thời gian tại đây)
    
    console.log(`Hệ thống đang đếm ngược mở khóa cho: ${username}`);
    setTimeout(async () => {
        try {
            await database.ref('users/' + username + '/isLocked').set(false);
            console.log(`Đã tự động mở khóa cho tài khoản: ${username}`);
        } catch (e) {
            console.error("Lỗi tự động mở khóa:", e);
        }
    }, delayTime);
}
...
const showToast = (msg) => {
    ...
    // XÓA class "show" ở dòng dưới đây
    const toastHTML = `
...
// Khởi tạo Toast với animation của Bootstrap là false 
// để nó không xung đột với CSS transition thủ công của bạn
const bsToast = new bootstrap.Toast(toastNode, { 
    delay: 4000,
    autohide: true,
    animation: false // Tắt animation mặc định của Bootstrap để dùng CSS của mình
});

bsToast.show();

// Đợi hiệu ứng trượt ra (500ms) kết thúc rồi mới xóa khỏi HTML
toastNode.addEventListener('hidden.bs.toast', () => {
    setTimeout(() => { 
        toastNode.remove(); 
    }, 500); // 500ms này phải khớp với thời gian transition trong CSS
});
};
